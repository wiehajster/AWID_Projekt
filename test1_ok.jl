...
for hn=hidden_neurons
	...
	layers=[]
	append!(layers, [fullyconnected(784, hn,ReLU())])
	append!(layers, [fullyconnected(hn, 10, Softmax())])
	net["Layers"]=layers;

	results = @benchmark train($net, $train_x, $test_x, $y_one_hot_train, $y_one_hot_test, $mb_size, $learning_rate, $epochs, $train_size);
...
df = DataFrame(d)
CSV.write("test1/test1_ok.csv", df)
