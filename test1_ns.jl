include("ns_reverse_accumulation.jl")
include("ns_optimizer.jl")
using MLDatasets
...
for hn=hidden_neurons
	println("hidden neurons:", hn)
	Wh  = Variable(randn(hn, input_neurons).*0.1, name="wh")
	bh = Variable(zeros(hn), name="bh")
	Wo  = Variable(randn(output_neurons, hn).*0.1, name="wo")
	...
	results = @benchmark train($graph, $input_x, $input_y, $y_output, $epochs, $optimizer, $Wh, $Wo, $bh, $bo)
...
df = DataFrame(d)
CSV.write("test1/test1_ns.csv", df)